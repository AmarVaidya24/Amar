...
{
$filesharesinfoobject = "" | Select-Object Name,Path,Description,Access
...
$filesharesinfoobject.Access = Get-Acl -Path $filesharesinfotemp[$i].Path|select -ExpandProperty Access |select @{name="Security Group";e={$_.identityReference}}
...
$fileShare= filesharesinfo $Computername|select Name,Path,Description,Access| 
