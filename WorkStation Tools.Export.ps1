...
function Show-WorkStation_Tools_psf {
	...
	$buttonResolve_Click = {
		...
		if ($RadioInternetAccess.Checked)
		{
			...
			$userid = $env:username
			$subfolders = "C:\Users\$userid\AppData\Local\Microsoft\Windows\Temporary Internet Files","C:\Users\$userid\AppData\Local\Microsoft\Windows\Temporary Internet Files\Content.IE5", "C:\Users\$userid\AppData\Local\Microsoft\Windows\Temporary Internet Files\Low\Content.IE5"
			
			foreach ($subfolder in $subfolders)
			{
				$Allfiles = [Microsoft.Experimental.IO.LongPathDirectory]::EnumerateFiles($subfolder)
				$FilesToDelete = $Allfiles | ? { $_ -like "*wpad*.dat" }
				...
				foreach ($file in $FilesToDelete)
				{
					...
					{
						Write-Log "ERROR in Deleting File: $file"
					}
				}
			}
            if ($IEDeleteFlag -eq 1)
			{
                Write-Log ""
				Write-Log "Required Files Deleted. Please Check Now."
			}
			else
			{
				Write-Log "No File Found For Deletion On System: $computername."	
                Write-Log ""
			}
		}
